...
class LFramework(nn.Module):
    def __init__(self, args, kg, mdl, ra):
        ...
        for epoch_id in range(self.start_epoch, self.num_epochs):
            ...
            # for example_id in range(0, len(train_data), self.batch_size):
                ...
                if 'critic_loss' in loss:
                    critic_loss.append(loss['critic_loss'])
                if 'sim_rates' in loss:
                    sim_rates.append(loss['sim_rates'])
                if 'hit_rates' in loss:
                    hit_rates.append(loss['hit_rates'])
                if self.run_analysis:
                    if rewards is None:
                        rewards = loss['reward']
                    else:
