...
# from utils.ops import unique_max, var_cuda, zeros_var_cuda, int_var_cuda, int_fill_var_cuda, var_to_numpy


def beam_search(pn, e_s, q, e_t, kg, ra, num_steps, beam_size, return_path_components=False):
    """
    ...
    # ra.actor.initialize_path(init_action, kg)
    ...
    # ra_log_action_prob = zeros_var_cuda(batch_size)
    ...
    for t in range(num_steps):
        ...
        # MA sample
        db_outcomes, _, _, bucket, _ = pn.transit(
            # e, obs, kg, refer_e, bucket, ra, use_action_space_bucketing=True, merge_aspace_batching_outcome=True)
            e, obs, ra, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=True)
        # 出来后不需要进行采样，就经过一个actor就可以了
        action_space, action_dist = db_outcomes[0]  # Main agent 动作空间及动作概率
        # => [batch_size*k, action_space_size]
        log_action_dist = log_action_prob.view(-1, 1) + ops.safe_log(action_dist)   # 新的动作分布
        ...
        action_offset = action_offset.long()
        if return_path_components:
        ...
        refer_e = ops.tile_along_beam(action[1].view(batch_size, -1)[:, 0], k)

        # RA sample
        # ra_action, next_e, bucket, _, _ = ra.perceive(e, obs, kg, merge_aspace_batching_outcome=True)
        # ra_db_outcomes, bucket, ra_entropy = ra.perceive(e, obs, kg, merge_aspace_batching_outcome=True)
        # ra_db_outcomes, ra_entropy, ra_inv_offset = ra.perceive(e, refer_e, obs, kg, bucket, merge_aspace_batching_outcome=True)
        # ra_action_space, ra_action_dist = ra_db_outcomes[0]  # RA动作空间及动作概率
        # ra_log_action_dist = ra_log_action_prob.view(-1, 1) + ops.safe_log(ra_action_dist)  # 新的动作分布
        # if t == num_steps - 1:
        #     ra_action, ra_log_action_prob, ra_action_offset = top_k_answer_unique(ra_log_action_dist, ra_action_space)
        # else:
        #     ra_action, ra_log_action_prob, ra_action_offset = top_k_action(ra_log_action_dist, ra_action_space)
        # ra_action_offset = ra_action_offset.long()
        # ra.actor.update_path(ra_action, kg, offset=ra_action_offset)
    ...
    return beam_search_output #, search_trace, [e_s, q, e_t]
