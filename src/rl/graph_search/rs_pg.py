...
# from emb.fact_network import get_conve_nn_state_dict, get_conve_kg_state_dict, \
#     get_complex_kg_state_dict, get_distmult_kg_state_dict
# from rl.graph_search.pg import PolicyGradient
# import utils.ops as ops
# from utils.ops import zeros_var_cuda


class RewardShapingPolicyGradient(PolicyGradient):
    def __init__(self, args, kg, pn, ra, fn_kg, fn, fn_secondary_kg=None):
        super(RewardShapingPolicyGradient, self).__init__(args, kg, pn, ra)
        ...
        # gpu 0 -> gpu 1
        # torch.load(args.xxx_state_dict_path, map_location={'cuda:0':'cuda:1'})
        fn_model = self.fn_model
        if fn_model in ['conve']:
            # fn_state_dict = torch.load(args.conve_state_dict_path)
            fn_state_dict = torch.load(args.conve_state_dict_path, map_location=lambda storage, loc: storage.cuda(args.gpu))
            ...
            fn_state_dict = torch.load(args.distmult_state_dict_path, map_location=lambda storage, loc: storage.cuda(args.gpu))
            fn_kg_state_dict = get_distmult_kg_state_dict(fn_state_dict)
        elif fn_model == 'complex':
            fn_state_dict = torch.load(args.complex_state_dict_path, map_location=lambda storage, loc: storage.cuda(args.gpu))
