...
# from emb.fact_network import ComplEx, ConvE, DistMult
# from emb.fact_network import get_conve_kg_state_dict, get_complex_kg_state_dict, get_distmult_kg_state_dict
# from emb.emb import EmbeddingBasedMethod
# from rl.graph_search.pn import GraphSearchPolicy
# from rl.graph_search.pg import PolicyGradient
# from rl.graph_search.rs_pg import RewardShapingPolicyGradient
# from rl.graph_search.ra import Representive_Agent
# from utils.ops import flatten
...
def initialize_model_directory(args, random_seed=None):
    ...
    args.data_dir = args.data_dir + '/NELL23K'
    # args.data_dir = args.data_dir + '/WD-singer'
    # args.data_dir = args.data_dir + '/FB15K-237-10'
    # args.data_dir = args.data_dir + '/FB15K-237'
...
def construct_model(args):
    ...
    if args.model in ['point', 'point.gc']:
        ...
        if fn_model == 'complex':
            ...
            fn_kg = KnowledgeGraph(fn_args)
        ra = Representive_Agent(args, fn_kg=fn_kg, fn=fn)
        pn = GraphSearchPolicy(args, fn_kg=fn_kg, fn=fn)
        lf = RewardShapingPolicyGradient(args, kg, pn, ra, fn_kg, fn)
    elif args.model == 'complex':
        fn = ComplEx(args)
...
def inference(lf):
    ...
    if args.compute_map:
        ...
        for r in relation_sets:
            ...
            mp = eval.link_MAP(test_data, pred_scores, labels, lf.kg.all_objects, verbose=True)
            mps.append(mp)
        map_ = np.mean(mps)
        ...
        eval.hits_and_ranks_by_relation_type(
        ...
        eval.hits_and_ranks_by_seen_queries(
...
def run_ablation_studies(args):
    ...
    for system in systems:
        ...
        pred_scores = lf.forward(dev_data, verbose=False)
        _, _, _, _, mrr = eval.hits_and_ranks(dev_data, pred_scores, lf.kg.dev_objects, verbose=True)
        if to_1_ratio == 0:
            to_m_mrr = mrr
            to_1_mrr = -1
        else:
            to_m_mrr, to_1_mrr = eval.hits_and_ranks_by_relation_type(
                dev_data, pred_scores, lf.kg.dev_objects, relation_by_types, verbose=True)
        seen_mrr, unseen_mrr = eval.hits_and_ranks_by_seen_queries(
            dev_data, pred_scores, lf.kg.dev_objects, seen_queries, verbose=True)
        mrrs[system] = {'': mrr * 100}
        ...
        _, _, _, _, mrr_full_kg = eval.hits_and_ranks(dev_data, pred_scores, lf.kg.all_objects, verbose=True)
        if to_1_ratio == 0:
            to_m_mrr_full_kg = mrr_full_kg
            to_1_mrr_full_kg = -1
        else:
            to_m_mrr_full_kg, to_1_mrr_full_kg = eval.hits_and_ranks_by_relation_type(
                dev_data, pred_scores, lf.kg.all_objects, relation_by_types, verbose=True)
        seen_mrr_full_kg, unseen_mrr_full_kg = eval.hits_and_ranks_by_seen_queries(
            dev_data, pred_scores, lf.kg.all_objects, seen_queries, verbose=True)
...
def export_error_cases(lf):
    ...
    print('Dev set performance:')
    pred_scores = lf.forward(dev_data, verbose=False)
    eval.hits_and_ranks(dev_data, pred_scores, lf.kg.dev_objects, verbose=True)
    eval.export_error_cases(dev_data, pred_scores, lf.kg.dev_objects, os.path.join(lf.model_dir, 'error_cases.pkl'))
